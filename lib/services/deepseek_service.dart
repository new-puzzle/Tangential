...
/// Deepseek API service for Whisper STT -> Deepseek -> TTS flow.
/// Supports vision/file uploads.
class DeepseekService {
  ...
  Future<String?> sendMessage(String message, {File? attachment}) async {
    if (_apiKey == null || _apiKey!.isEmpty) {
      debugPrint('Deepseek API key not set');
      return null;
    }
    ...
    try {
      ...
      // Add file attachment if present (Deepseek supports vision)
      if (attachment != null) {
        final mimeType =
      ...
      // Use deepseek-chat for text, deepseek-vision if image attached
      final model =
          attachment != null &&
              (lookupMimeType(attachment.path) ?? '').startsWith('image/')
          ? 'deepseek-vision'
          : 'deepseek-chat';
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('Deepseek response: $responseText');
      ...
      debugPrint('Error sending message to Deepseek: $e');
      return null;
    }
  }
  ...
  Future<String?> sendMessageWithImage(
    ...
    try {
      ...
      debugPrint('Error sending image to Deepseek: $e');
      return null;
    }
  }
  ...
  void clearHistory() {
    ...
    debugPrint('Deepseek conversation history cleared');
  }
